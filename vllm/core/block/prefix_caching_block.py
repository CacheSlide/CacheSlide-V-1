...
class PrefixCachingBlockAllocator(BlockAllocator):
    ...
    def __init__(
        ...
        spill_cb: Optional[Callable[[int, int], str]] = None
    ):
        ...
        self._ssd_index: Dict[PrefixHash, str] = {}   # content_hash -> ssd_location
        self._spill_cb = spill_cb                     # (block_id, content_hash) -> location
    ...
    def allocate_immutable_block(
        ...
        probe = self._block_pool.init_block(
            prev_block=prev_block,
            token_ids=token_ids,
            block_size=self._block_size,
            physical_block_id=None,
        ...
        assert probe.content_hash is not None
        ...
        if cached_block_id is not None:
            self.metric_data.query(hit=True)
            probe.block_id = cached_block_id
            self._incr_refcount_cached_block(probe)
            return probe

        # 2) RAM miss -> 看 SSD index
        loc = getattr(self, "_ssd_index", {}).get(content_hash)  # self._ssd_index: Dict[int, str]
        if loc is not None and getattr(self, "_load_cb", None) is not None:
            # probe 不再需要（我们要走“新分配+回填”路径）
            self._block_pool.free_block(probe)

            # 2.1 分配一个新的物理 block（refcount=1，tracker active，computed=False）
            block = self.allocate_mutable_block(prev_block, extra_hash=extra_hash)

            # 2.2 走现有逻辑把它变成 immutable + 建立 _cached_blocks 映射
            #     （promote_to_immutable_block 会把 content_hash -> block_id 写进 _cached_blocks）
            block.append_token_ids(token_ids)

            # 2.3 回填 KV：让 worker/engine 把 loc 里的 KV 读回到这个 block_id
            assert block.block_id is not None
            self._load_cb(block.block_id, content_hash, loc)  # 你需要实现/注入这个回调

            # 2.4 SSD 回填的块此时已经“computed”，否则 prefix skip 不会生效
            block.computed = True
            self._block_tracker[block.block_id].computed = True

            # 统计：RAM miss（你也可以单独加一个 ssd_hit metric）
            self.metric_data.query(hit=False)
            return block

        # 3) RAM miss + SSD miss -> 原逻辑：新算一份
        self.metric_data.query(hit=False)
        self._block_pool.free_block(probe)
    ...
    def _maybe_allocate_evicted_block_id(self) -> Optional[BlockId]:
        ...
        if self._spill_cb is not None:
            loc = self._spill_cb(block_id, content_hash_to_evict)
            self._ssd_index[content_hash_to_evict] = loc
